...
const IPAddressFilter = async (req, res, next) => {
    try {
        ...
        if ((filter_mode === 'whitelist' && !isInSingle) || (filter_mode === 'blocklist' && isInSingle))
            return next();

        const isInRange = await Promise.all(rangeIPs.map(async range => {
            return isIPInRange(clientIP, range);
        })).then(results => results.some(result => result));

        if ((filter_mode === 'whitelist' && !isInRange) || (filter_mode === 'blocklist' && isInRange))
